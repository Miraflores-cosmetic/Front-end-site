...
import { createAddressService, updateAddress } from '@/graphql/queries/address.service';
import { AddressInput, AddressTypeEnum } from '@/graphql/types/address.types';
...
import {
    addressInfoFromGuestInput,
    GUEST_SHIPPING_ADDRESS_ID,
    saveGuestShippingAddress,
} from '@/utils/guestShippingAddress';

function isRuPostalCode(raw: string | undefined | null): boolean {
...
function parseDeliveryMethodFromStreet2(street2: string | undefined | null): DeliveryMethodId {
    ...
    if (vm?.carrier === 'cdek') {
        return vm.dropoff === 'courier' ? 'cdek_courier' : 'cdek_pvz';
    }
    const s = street2 || '';
...
const AddressDrawer: React.FC = () => {
    ...
    /** Код ПВЗ СДЭК (обязателен для cdek_pvz) */
    const [cdekPvzDraft, setCdekPvzDraft] = useState<{
        id: string;
        cityCode: string;
        lat: string;
        lon: string;
    ...
    useEffect(() => {
        ...
        if (seed) {
            ...
            if (ym?.carrier === 'cdek' && ym.dropoff === 'pvz' && ym.pvz) {
                setCdekPvzDraft({
                    id: ym.pvz,
    ...
    const handleSubmit = async () => {
        ...
        const street = formData.streetAddress1.trim();
        if (!city) {
            setFieldErrors((prev) => ({ ...prev, city: 'Укажите город' }));
            toast.error('Укажите город');
            return;
        }
        if (!street) {
            setFieldErrors((prev) => ({ ...prev, streetAddress1: 'Укажите улицу и дом' }));
        ...
        setFieldErrors({});
        try {
            const payload = { ...formData, city, streetAddress1: street };

            payload.apartment = (payload.apartment || '').trim();
            ...
            if (payload.country === 'RU') {
                ...
                if (deliveryMethod === 'cdek_pvz') {
                    if (!cdekPvzDraft?.id) {
                        toast.error('Выберите пункт выдачи СДЭК в списке или на карте');
                        setIsLoading(false);
                        return;
                    }
                    payload.streetAddress2 = buildStreetAddress2WithMeta(
                        {
                            carrier: 'cdek',
                            lon: cdekPvzDraft.lon || '',
                            lat: cdekPvzDraft.lat || '',
                            pvz: cdekPvzDraft.id,
                            cid: cdekPvzDraft.cityCode || '',
                            dropoff: 'pvz',
                        },
                        tail,
                        COMMENT_MAX,
                    );
                } else if (deliveryMethod === 'cdek_courier') {
                    if (!isRuPostalCode(payload.postalCode)) {
                        setFieldErrors((prev) => ({
                            ...prev,
                            postalCode: 'Укажите 6-значный индекс',
                        }));
                        toast.error('Укажите индекс для курьера СДЭК');
                        setIsLoading(false);
                        return;
                    }
                    payload.streetAddress2 = buildStreetAddress2WithMeta(
                ...
                } else if (deliveryMethod === 'yandex_pvz') {
            ...
            payload.streetAddress2 = clampAddressField(payload.streetAddress2, COMMENT_MAX);
            ...
            if (!isAuth) {
                const guestAddress = addressInfoFromGuestInput(
                    payload,
                    editingAddressId || GUEST_SHIPPING_ADDRESS_ID,
                );
                saveGuestShippingAddress(guestAddress);
                toast.success(
                    isEditMode ? 'Адрес обновлён' : 'Адрес сохранён для оформления заказа',
                );
                handleClose();
                return;
            }
            ...
            <div className={styles.contentWrapper}>
                ...
                <article className={styles.formFields}>
                                    ...
                                    <CdekPvzList
                                        onChoose={(info: CdekPvzInfo) => {
                                            setCdekPvzDraft({
                                                ...
                                                lat:
                                                    info.lat != null && Number.isFinite(info.lat)
                                                        ? String(info.lat)
                                                        : '',
                                                lon:
                    ...
                    <Input
                        ...
                        fieldError={fieldErrors.apartment}
                    />
